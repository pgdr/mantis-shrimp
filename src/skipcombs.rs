pub struct SkippableCombinations<I> where I: Iterator   {
    data: Vec<I::Item>,
    pointers: Vec<usize>,
    stop: Vec<usize>,
    last_moved: usize,
...
impl<I> SkippableCombinations<I> where I: Iterator {
    fn new(data:Vec<I::Item>, k:usize) -> Self {
        let n = data.len();
        if n < k {
            return SkippableCombinations{ data: vec![],  pointers: vec![], stop: vec![], finished: true, last_moved: 0 };
        } 
        ...
        SkippableCombinations { data, pointers, stop, finished: false, last_moved: usize::MAX }
    }
          ...
          T: Clone
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let current = self.pointers.iter().skip(1).map(|&ix| self.data[ix].clone()).collect();
        ...
        if ix == 0 { // We hit the guard element
            self.finished = true;
        } else {
            // 2) Advance pointer 
            self.pointers[ix] += 1;
...
pub trait SkippableCombinationsIter<I> where I: Iterator {
    fn combinations_skippable(self, k:usize) -> SkippableCombinations<I>;
}

impl<I> SkippableCombinationsIter<I> for I where I: Iterator + Sized
{
    fn combinations_skippable(self, k:usize) -> SkippableCombinations<I> {
